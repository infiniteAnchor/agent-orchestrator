...
describe("createRemoteMuxBridge", () => {
	...
	it("keeps a close tombstone when the pending queue overflows before subscribe", async () => {
		let constructed: FakeSocket | undefined;
		const bridge = createRemoteMuxBridge(() => "/tmp", {
			fetchImpl: vi.fn(async () =>
				new Response(JSON.stringify({ hostId: "h_lab", apiVersion: 1 }), { status: 200 }),
		...
		const { connectionId } = await bridge.connect(sender);
		await vi.waitFor(() => constructed?.readyState === FakeSocket.OPEN);
		for (let i = 0; i < MAX_PENDING_MUX_EVENTS + 1; i += 1) {
			constructed?.emit("message", Buffer.from(`frame-${i}`), false);
		}
		expect(sender.events).toEqual([]);
		bridge.subscribe(sender, connectionId);
		// Overflow drops prior payloads (including open) but keeps a terminal close.
		expect(sender.events.map((e) => (e.event as { type: string }).type)).toEqual(["close"]);
		const close = sender.events[0]?.event as { reason?: string };
		expect(close.reason).toMatch(/overflow/i);
	});
